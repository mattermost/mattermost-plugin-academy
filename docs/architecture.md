...
- **Completion yardstick** — whether a guide is finished is computed against the server catalog (plugin-gated modules omitted unless that plugin is running, or Test Mode is on). Client-supplied module lists are ignored. Completed IDs that are not in that current yardstick are not stored. Progress responses filter stored IDs to that same yardstick, so a module gated behind a since-disabled plugin stops counting toward progress without being erased.
