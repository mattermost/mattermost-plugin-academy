...
- **Completion yardstick** — whether a guide is finished is computed against the server catalog, with plugin-gated modules omitted unless that plugin is running (or Test Mode is on) so the guide stays finishable. Client-supplied module lists are ignored, and completed IDs outside that yardstick are not stored.
